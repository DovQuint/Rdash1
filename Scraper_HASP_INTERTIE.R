...
# 1. Define HASP INTERTIE Scraper Function
#   Function Inputs:
...
FN_Scrape_PRC_HASP_INTERTIE <-  function(input_query_name,
                                   input_start_date,
...
# 3. Execute Function
FN_Scrape_PRC_HASP_INTERTIE(input_query_name = input_query_name,
...
file_list <- file.info(list.files("C:/R/Dash1/CSV/HASP_INTERTIE", full.names = T))
most_recent_file <- rownames(file_list)[which.max(file_list$mtime)]
most_recent_file

df_read_HASP_INTERTIE <- read.csv(most_recent_file) %>% 
  mutate(OPR_DT = lubridate::as_date(OPR_DT)) 

df_recent_write_HASP_INTERTIE <- df_read_HASP_INTERTIE %>% 
...
csv_string <- "C:/R/Dash1/CSV/REACTIVE/DASH_HASP_INTERTIE.csv"
write.csv(df_recent_write_HASP_INTERTIE, csv_string, row.names = FALSE)
