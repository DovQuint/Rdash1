...
# 1. Define HASP Nomogram Scraper Function
#   Function Inputs:
...
FN_Scrape_PRC_HASP_NOMOGRAM <-  function(input_query_name,
                                   input_start_date,
...
# 3. Execute Function
FN_Scrape_PRC_HASP_NOMOGRAM(input_query_name = input_query_name,
...
file_list <- file.info(list.files("C:/R/Dash1/CSV/HASP_NOMOGRAM", full.names = T))
most_recent_file <- rownames(file_list)[which.max(file_list$mtime)]
most_recent_file

df_read_HASP_NOMOGRAM <- read.csv(most_recent_file) %>% 
  mutate(OPR_DT = lubridate::as_date(OPR_DT)) 

df_recent_write_HASP_NOMOGRAM <- df_read_HASP_NOMOGRAM %>% 
...
csv_string <- "C:/R/Dash1/CSV/REACTIVE/DASH_HASP_NOMOGRAM.csv"
write.csv(df_recent_write_HASP_NOMOGRAM, csv_string, row.names = FALSE)
