...
#   dynamic CSV file to be read by reactive Shiny function within flex dashboard. The functions are 
#   aggregated and executed within a perpetual loop to run every 5 mins
...
# 1. Define RTPD LMP Scraper Function
#   Function Inputs:
#   - input_query_name =  name of report being queried (i.e. PRC_CNSTR)
...
FN_Scrape_PRC_RTPD_LMP <-  function(input_query_name,
                                         input_start_date,
                                         input_version_number,
                                         input_select_columns) {
  
  #  INTERNAL DEBUGGER - FUNCTION INPUTS #
  # input_query_name <- "PRC_CNSTR"
  # input_start_date <- "2024-05-01"
  # input_version_number <- 1
  # input_sql_data_source <- "CAISO_PRICES"
  # input_select_columns <- c("OPR_DT", "OPR_HR", "OPR_INTERVAL", "TI_ID", "TI_DIRECTION", "CONSTRAINT_CAUSE", "MW")
...
# 3. Execute Function
FN_Scrape_PRC_RTPD_LMP(input_query_name = input_query_name,
...
file_list <- file.info(list.files("C:/R/Dash1/CSV/RTPD_LMP", full.names = T))
most_recent_file <- rownames(file_list)[which.max(file_list$mtime)]
most_recent_file

df_read_RTPD_LMP <- read.csv(most_recent_file) %>% 
  mutate(OPR_DT = lubridate::as_date(OPR_DT)) 

df_recent_write_RTPD_LMP <- df_read_RTPD_LMP %>% 
  filter(OPR_DT == max(OPR_DT)) %>% 
  filter(OPR_HR == max(OPR_HR)) %>%
  filter(OPR_INTERVAL == max(OPR_INTERVAL)) %>% 
  dplyr::mutate(SHADOW_PRC = round(MW, 2)) %>% 
  dplyr::mutate(HE_INT = paste0(OPR_HR,"-", OPR_INTERVAL)) %>% 
  dplyr::select(-c(X, CONSTRAINT_CAUSE, OPR_DT, MW, OPR_INTERVAL, MARKET_RUN_ID)) 


# 5. Select columns and write to CSV
csv_string <- "C:/R/Dash1/CSV/REACTIVE/DASH_RTPD_LMP.csv"
write.csv(df_recent_write_RTPD_LMP, csv_string, row.names = FALSE)
