...
# 1. Define RTD INTERTIE Scraper Function
#   Function Inputs:
...
FN_Scrape_PRC_RTD_INTERTIE <-  function(input_query_name,
                                         input_start_date,
                                         input_version_number,
                                         input_select_columns) {
  
  #  INTERNAL DEBUGGER - FUNCTION INPUTS #
  # input_query_name <- "PRC_RTM_FLOWGATE"
  # input_start_date <- "2024-05-01"
  # input_version_number <- 1
  # input_sql_data_source <- "CAISO_PRICES"
  # input_select_columns <- c("OPR_DT", "OPR_HR", "OPR_INTERVAL", "TI_ID", "TI_DIRECTION", "CONSTRAINT_CAUSE", "PRC")
...
# 3. Execute Function
FN_Scrape_PRC_RTD_INTERTIE(input_query_name = input_query_name,
...
file_list <- file.info(list.files("C:/R/Dash1/CSV/RTD_INTERTIE", full.names = T))
most_recent_file <- rownames(file_list)[which.max(file_list$mtime)]
most_recent_file

df_read_RTD_INTERTIE <- read.csv(most_recent_file) %>% 
  mutate(OPR_DT = lubridate::as_date(OPR_DT)) 

df_recent_write_RTD_INTERTIE <- df_read_RTD_INTERTIE %>% 
  ...
  dplyr::select(-c(X, CONSTRAINT_CAUSE, OPR_DT, PRC, OPR_INTERVAL, MARKET_RUN_ID)) 


# 5. Select columns and write to CSV
csv_string <- "C:/R/Dash1/CSV/REACTIVE/DASH_RTD_INTERTIE.csv"
write.csv(df_recent_write_RTD_INTERTIE, csv_string, row.names = FALSE)
