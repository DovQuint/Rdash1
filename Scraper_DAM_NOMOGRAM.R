...
# 1. Define DAM Nomogram Scraper Function
#   Function Inputs:
...
FN_Scrape_PRC_DAM_NOMOGRAM <-  function(input_query_name,
                                   input_start_date,
                                   input_version_number,
                                   input_select_columns) {
  
  #  INTERNAL DEBUGGER - FUNCTION INPUTS #
  # input_query_name <- "PRC_NOMOGRAM"
...
# 3. Execute Function
FN_Scrape_PRC_DAM_NOMOGRAM(input_query_name = input_query_name,
...
file_list <- file.info(list.files("C:/R/Dash1/CSV/DAM_NOMOGRAM", full.names = T))
most_recent_file <- rownames(file_list)[which.max(file_list$mtime)]
most_recent_file

df_read_DAM_NOMOGRAM <- read.csv(most_recent_file) %>% 
  mutate(OPR_DT = lubridate::as_date(OPR_DT)) %>% 
  filter(OPR_DT == lubridate::today())

df_recent_write_DAM_NOMOGRAM <- df_read_DAM_NOMOGRAM %>% 
...
csv_string <- "C:/R/Dash1/CSV/REACTIVE/DASH_DAM_NOMOGRAM.csv"
write.csv(df_recent_write_DAM_NOMOGRAM, csv_string, row.names = FALSE)
